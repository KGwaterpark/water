...
<c:set var="m_id" value="${memberLogin.m_id}" />
	...
	<c:if test="${ocean.m_id eq m_id && ocean.state eq 'N'}">
		<button class="bababa" id = "sangsese"  ${ocean.ocbook_id}>
     <table>
	<tr>
   ...
   <br><br>
	...
	<c:forEach var="resort" items="${resortlist}">
		...
		<button class="bababa" id = "sangsese" ${resort.rebook_id}>
		<table>
			<tr>
			 	<td >예약</td><td> : </td><td><span>${ocean.state eq 'Y' ? '예약' : '예약취소'}</span></td>
	...
	<c:forEach var="mypackage" items="${packagelist}">
		...
		<button class="bababa" id = "sangsese" ${mypackage.pabook_id}'">
		<table>
			<tr>
